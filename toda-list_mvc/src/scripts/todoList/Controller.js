...
const Controller = {
    ...
    formHandler(event){
        ...
        try{
            ...
            alert('Cannot save data - DB_KEY is full');
        }
    ...
    viewHandler(event){
        ...
        const todoItemId = Number(btn.closest(`[data-id]`).getAttribute(`data-id`));
        const todaItemData = Model.getById(todoItemId);

        View.showInfo(todaItemData);
    },
    ...
    validateSelector(selector){
        ...
        if(selector.trim() === '')
            throw new Error('selector should not be ampty');
